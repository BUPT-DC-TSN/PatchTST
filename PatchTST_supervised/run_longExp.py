...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_hour_sin', type=int, default=0, help='0 use hour sin and cos as time feature, if 1 use no time feature')
    parser.add_argument('--draw', type=int, default=0, help='0 not draw, if 1 draw')
    ...
    if args.is_training == 1:
        ...
        exp = Exp(args)  # set experiments
        print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
        exp.test(setting, args.checkpoint_path, test=1)
        torch.cuda.empty_cache()
