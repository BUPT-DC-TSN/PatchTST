...
class Dataset_Infer_No_ot(torch.utils.data.Dataset):
    def __init__(self, root_path, data_path, seq_len, pred_len):
        ...
        # self.data_mean = self.df_data['OT'].mean()
        # self.data_std = self.df_data['OT'].std()
        
        # self.data_mean = 3370001.158234162
        # self.data_std = 461434.31922590954
        
        df_ot = pd.read_csv('/mnt/e/timer/PatchTST/dataset/0117/pred_test/pred.csv')
        
        self.data_mean = df_ot['OT'].mean()
        self.data_std = df_ot['OT'].std()
    ...
    def inverse_output(self, pred):
        return pred * self.data_std + self.data_mean
...
class Infer:
    ...
    def draw(self, save_path, pic_name="pred_and_original.png"):
        ...
        plt.legend()
        save_pic_path = os.path.join(save_path, pic_name)
        plt.savefig(save_pic_path, dpi=300)
        print(f"image saved in {save_pic_path}")
