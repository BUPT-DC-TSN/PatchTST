...
class Dataset_Custom_old(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.features == 'M' or self.features == 'MS':
            cols_data = df_raw.columns[1:]
            df_data = df_raw[cols_data][border1:border2]
        ...
        if self.timeenc == 0:
            # 提取基本时间特征
            # 我们的数据集按天为周期的特征比较明显(中午强晚上弱等等)，因此对小时正余弦编码
            df_stamp['hour'] = df_stamp['date'].dt.hour
            df_stamp['minute'] = df_stamp['date'].dt.minute
            df_stamp['seconds'] = df_stamp['date'].dt.second
            df_stamp['fractional_seconds'] = (df_stamp['date'] - df_stamp['date'].dt.floor('s')).dt.total_seconds()
            
            seconds_in_day = df_stamp['hour'] * 3600 + df_stamp['minute'] * 60 + df_stamp['seconds'] + df_stamp['fractional_seconds']
            day_seconds = 24 * 60 * 60
            # 计算正弦和余弦特征
            df_stamp['Seconds_sin'] = np.sin(2 * np.pi * seconds_in_day / day_seconds)
            df_stamp['Seconds_cos'] = np.cos(2 * np.pi * seconds_in_day / day_seconds)
            
            df_stamp = df_stamp.drop(['date', 'hour', 'minute', 'seconds', 'fractional_seconds'], axis=1)
            data_stamp = df_stamp.values
            df_stamp = df_stamp.reset_index(drop=True)
            
            df_data = pd.concat([df_stamp, df_data], axis=1)
            print(df_data.head())
        ...
        # train_data = df_data.drop(['OT'], axis=1)
        if self.scale:
            train_data = df_data
            self.scaler.fit(train_data.values)
...
class Dataset_5G_NoOT(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.features == 'M' or self.features == 'MS':
            ...
            df_data = df_raw[cols_data][border1:border2]
            df_data = df_data.reset_index(drop=True)
            df_data = df_data.drop([self.target], axis=1)
        elif self.features == 'S':
            df_data = df_raw[[self.target]]
        ...
        # 删掉ot
        self.data_x = data
        self.output_mean = df_raw[self.target].values.mean()
        self.output_std = df_raw[self.target].values.std()
        self.data_y = (df_raw[self.target].values[border1:border2] - self.output_mean) / self.output_std
        # self.data_y = df_raw[self.target].values[border1:border2]*1e-7
...
class Dataset_Pred(Dataset):
    def __init__(self, root_path, flag='pred', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, inverse=False, timeenc=0, freq='s', draw=True, cols=None):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        ...
        df_raw = df_raw[['date'] + cols + [self.target]]
        if self.draw:
            border1 = int(len(df_raw) - 0.7 * len(df_raw))
            border1 = len(df_raw) - self.seq_len - 300
        else:
        ...
        #     else:
        #         self.data_y = data[:self.seq_len+self.pred_len]
        self.data_x = data
        self.data_y = data
        self.data_stamp = data_stamp
    ...
    def __getitem__(self, index):
        ...
        seq_y_mark = self.data_stamp[r_begin:r_end]
        seq_x_mark = self.data_stamp[r_begin: r_end]
...
class Dataset_Pred_old(Dataset):
    def __init__(self, root_path, flag='pred', size=None,
                 features='S', data_path='ETTh1.csv',
    ...
    def __read_data__(self):
        ...
        if self.inverse:
            ...
            self.data_y = data[border1:border2]
        self.data_stamp = data_stamp
        print(len(self.data_x), self.seq_len)
