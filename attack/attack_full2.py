...
full_df = pd.read_csv("/mnt/e/timer/attack/fixed_data_new.csv")
...
features_df = full_df.drop(['is_attack', 'attack_value', 'data', 'attack_type', 'attack_log_value'], axis=1)
...
for epoch in range(100):
    ...
    if val_acc > best_acc:
        best_acc = val_acc
        torch.save(cls_model.state_dict(), "cls_model_full2.pth")
        counter = 0
    else:
        counter += 1
...
cls_model.load_state_dict(torch.load("cls_model_full2.pth"))
cls_model.eval()
...
def prepare_regression_features(df, window_size=20):
    ...
    for i in range(window_size, len(df)):
        ...
        current = df.iloc[i][['master_offset', 'path_delay', 's2_freq', 'adjusted_Offset', 'attack_type']].values
...
for epoch in range(200):
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        torch.save(reg_model.state_dict(), "reg_model_2.pth")
        counter = 0
    else:
        counter += 1
...
reg_model.load_state_dict(torch.load("reg_model_2.pth"))
reg_model.eval()
...
joblib.dump(cls_scaler, 'cls_feature_scaler.pkl')
joblib.dump(target_scaler, 'reg_target_scaler.pkl')
joblib.dump(reg_model, 'reg_feature_scaler.pkl')
